...
\subsection{Andere stochastische veranderlijken}
\label{sec:andere-stoch-verand}
...
\begin{ei}
  \begin{itemize}
  \item $Var[aX] = a^{2}Var[X]$ met $a$ een constante
  \item $Var[X+b] = Var[X]$ met $b$ een constante
  \item $Var[b] = 0$ met $b$ een constante
  \end{itemize}
...
\begin{ei}
  ...
  \item $Vay[XY] = Var[X]Var[Y] + E[X]^{2}Var[Y] + Var[X]E[Y]^{2}$
  \end{itemize}
\TODO{bewijs p 45}
\end{ei}
...
\begin{st}
  ...
  Zij $X$ een stochastische variabele en $\phi:\mathbb{R} \rightarrow \mathbb{R}$ een stochastische variabele zodat $E[\phi(X)]$ bestaat.
  \begin{enumerate}
...
\subsection{Continue verdelinen}
\label{sec:continue-verdelinen}

\subsubsection{Continue uniforme verdelingen}
\label{sec:cont-unif-verd}

\begin{de}
  De stochastische variabele $X$ volgt een continue verdeling op $[a,b]$ als $X$ de volgende dichtheid heeft.
