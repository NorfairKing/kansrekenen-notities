...
\begin{examenvraag}{Tussentijdse Toets 2015}
  ...
  \begin{ex-antwoord}
    ...
    \item Toepassing van de wetten.\\
      Zoals typisch bij een ``Wie is verantwoordelijk?'' vraag, proberen we de stelling van Bayes toe te passen:
      ...
      De kans $P(S|B)$ dat $S$ dan faalt is dus $p_{A}+p_{D}$.
      Als $B$ zeker niet gefaald heeft, valt $B$ gewoon weg te denken.
      De kans $P(S|B^{C})$ dat $S$ dan faalt is dus $p_{D} + p_{A}p_{C}$.
      We kunnen de formule nu herschrijven:
      \[ P(B|S) = \frac{(p_{A}+p_{D})p_{B}}{(p_{A}+p_{D})p_{B} + (p_{D} + p_{A}p_{C})(1-p_{B})} \]
      Hierin is alles behalve $p_{B}$ gekend.
      We vormen dit om:
      \begin{align*}
        P(B|S) &= \frac{(p_{A}+p_{D})p_{B}}{(p_{A}+p_{D})p_{B} + (p_{D} + p_{A}p_{C})(1-p_{B})}\\
        p_{B} &= \frac{P(B|S)(p_{D} + p_{A}p_{C})}{p_{A}+p_{D}+ P(B|S)(p_{D} + p_{A}p_{C}) - P(B|S)(p_{A}+p_{D})}\\
        p_{B} &= \frac{(0.1)(0.01+(0.05)(0.02))}{0.05+0.01 + (0.1)(0.01 + (0.05)(0.02)) - (0.1)(0.05+0.01)}\\
        p_{B} &= 0.019963702359346643
      \end{align*}
    \end{itemize}
  \end{ex-antwoord}
\end{examenvraag}
